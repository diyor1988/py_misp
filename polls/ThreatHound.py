...
class misp_threat_intelligence():
    ...
    def fetch_feed_content(self, feed_id):
        ...
        # $how_result = "Fetching threat from feed{}-{}".format(self.feed_id, self.feed_name)
...
def get_threat_feeds_db() :

    return threat_feeds_db
...
def cacheModeThread(cache_mode, args, idx) :
...
def get_feeds(feed_id):
    ...
    feed_name = misp_instance.retrieve_feed_name_by_id(feed_id) 
    feed_content = misp_instance.fetch_feed_content(feed_id)
    # feed_entities_result =  ""

    # feeds_preview_url = MISP_SERVER + '/feeds/previewIndex/{}/page:'.format(feed_id)
    # try:
    #     rep = requests.get(feeds_preview_url, headers = req_header, verify = False)
    #     feed_entities_result = json.loads(rep.content)
    #     feed_entity_num = len(feed_entities_result)
    #     cprint_cyan("feed{}-{}中共获取到{}条数据".format(feed_id, feed_name, feed_entity_num))
    # except Exception as e:
    #     cprint_red("feed请求失败")
    #     # cprint_red(e)

    # return feed_entities_result

    return feed_content
    # return misp_instance.feed_entities
